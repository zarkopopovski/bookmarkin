...
type Bookmark struct {
	...
	IconEncoded   string `json:"icon_encoded"`
}
...
func (bookmark *Bookmark) CreateNewBookmark(dbConnection *DBConnection, userID string) bool {
	...
	query := "INSERT INTO bookmarks(id, user_id, bookmark_url, bookmark_title, bookmark_icon, bookmark_icon_base64, bookmark_group) VALUES('" +
		bookmarkID + "','" + userID + "','" + bookmark.BookmarkUrl + "','" + bookmark.BookmarkTitle + "','" + bookmark.IconURL + "','" + bookmark.IconEncoded + "','" + bookmark.BookmarkGroup + "')"
...
func (bookmark *Bookmark) ListAllBookmarks(dbConnection *DBConnection, userID string) []*Bookmark {
	query := "SELECT b.id, b.bookmark_url, b.bookmark_title, b.bookmark_icon, b.bookmark_icon_base64, g.group_name FROM bookmarks b INNER JOIN groups g ON b.bookmark_group=g.id AND b.user_id='" + userID + "'"
...
func (bookmark *Bookmark) ListAllBookmarksInGroup(dbConnection *DBConnection, groupID string, userID string) []*Bookmark {
	query := "SELECT id, bookmark_url, bookmark_title, bookmark_icon, bookmark_icon_base64 FROM bookmarks WHERE bookmark_group='" + groupID + "' AND user_id='" + userID + "'"
